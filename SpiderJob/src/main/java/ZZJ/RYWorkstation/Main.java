...
//		jobBeanList = JobBeanUtils.LoadJobBeanList("BigData_"+year+month+day+"_Ori.txt");
		jobBeanList = JobBeanUtils.LoadJobBeanList("BigData_20191126_Ori.txt");
...
//		
//		//清洗后的本地备份
//		JobBeanUtils.SaveJobBeanList(jobBeanList,"BigData_20191124_Clean.txt");
//		
//		//将清洗后的数据存入数据库
//		ConnectMySQL cm = new ConnectMySQL();
//		Connection conn = cm.getConn();
//		//调用数据库工具类将JobBean容器存入数据库
//		DBUtils.insert(conn, "jobInfo_test", jobBeanList);
	}
