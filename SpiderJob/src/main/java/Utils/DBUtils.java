...
//					System.out.println("重复数据不插入");
	...
	public static List<JobBean> select(Connection conn, String tableName){
		...
		String command = "select jobName,company,address,salary,date,exp,edu,offerNumber,jobInfo,companyType,staffNumber,companyOrientation from "+tableName;
		try {
			ps = conn.prepareStatement(command);
			rs = ps.executeQuery();
			while(rs.next()) {
